...
export class Products extends Component {
    ...
    createProduct = (product) => {      
        return(
            <div className='flex justify-between first:pt-4 cursor-pointer hover:bg-gray-100 transition ease-in-out duration-200 group'>
                <div className='flex'>
                    <img className='w-[200px] object-scale-down' src={product.Image} alt='Мотор'/>
                    <div className=''>
                        <h1 className='pt-8 font-medium text-black text-2xl group-hover:text-green-600'>{product.Name}</h1>
    ...
    render() {
        return (
            <div className='bg-white shadow-lg flex-grow ring-1 ring-gray-300 rounded-lg h-[4000px]'>
                {this.state.fetchedData.map((product) => {return this.createProduct(product)})}
            </div>
        )
    }
}
